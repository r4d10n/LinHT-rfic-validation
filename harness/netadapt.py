...
from common import REPO_ROOT, StageError
...
def debrace(line: str) -> str:
    """Strip ngspice {expr} braces; simulators take bare expressions.
    Braces are removed without adding whitespace so k={v} stays k=v."""
    ...
    for ln in text.splitlines():
        ...
        if target == "ads":
            ...
            if m and not low.startswith("."):
                ...
                sub, nodes, params = toks[-1], [], []
                # subckt name is the token right after the node list;
                # everything after it that contains '=' is a param
                seen_param = False
                body = []
                for t in toks:
                    if "=" in t or seen_param:
                        ...
                        body.append(t)
                # last non-param token is the subckt name
                nodes = body[:-1]
                subname = body[-1]
...
def gen_ads(doc: dict, corner: str, extra_defs: list[str]) -> list[str]:
    ...
    if doc["params"]:
        o.append(emit_params_ngstyle(doc["params"]))
    o += extra_defs
